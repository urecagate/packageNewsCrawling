...
def scrape_site1(driver):
    ...
    for cand in candidates:
        ...
        try:
            ...
            if pub_date == today or pub_date == yesterday:
                ...
                print(f"scrape_site1 - 그리드 항목 날짜 불일치: {cand['title']} ({pub_date} != {today} 또는 {yesterday}). 그리드 처리를 종료하고 리스트 영역으로 이동합니다.")
                break
        except Exception as e:
    ...
    if not valid_candidates:
        print("scrape_site1 - 그리드 영역에 오늘/어제 날짜 기사가 없으므로 리스트 영역 처리 시작")
    else:
    ...
    for cand in candidates:
        ...
        try:
            ...
            if pub_date == today or pub_date == yesterday:
                ...
                print(f"scrape_site1 - 리스트 항목 날짜 불일치: {cand['title']} ({pub_date} != {today} 또는 {yesterday}). 리스트 영역 처리를 종료합니다.")
                break
        except Exception as e:
            print("scrape_site1 - 리스트 날짜 파싱 오류:", cand["date_text"], e)

    print(f"scrape_site1 - 오늘/어제 날짜 기사 후보 수: {len(valid_candidates)}개")

    # 5. 오늘/어제 날짜 후보들에 대해 개별 기사 페이지에서 본문 추출
